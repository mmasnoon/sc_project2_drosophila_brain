...
Project 2 reanalysis of Baker et al. (2021, *Genome Research*), "The Drosophila brain on cocaine at single-cell resolution" (GEO: [GSE152495](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE152495)), using a Scanpy/Leiden pipeline.

## Overview

Adult *Drosophila melanogaster* brains were profiled by 10x Genomics scRNA-seq after acute consumption of sucrose (control) or sucrose supplemented with cocaine, in both sexes and in duplicate. This repo reproduces the QC → normalization → clustering → marker annotation → differential expression → pathway enrichment pipeline and compares the results against the original publication's ~36-cluster atlas and reported male-biased cocaine response.

**Status:** Draft (Milestone 1) — see [Report](#report--deliverables) for current findings and known limitations.
...
| Step | Section |
|---|---|
| 8.1 | Load 10x data, build unified AnnData |
| 8.2 | QC & filtering |
| 8.3 | Normalization & scaling |
| 8.4 | PCA, UMAP, Leiden clustering |
| 8.5 | Marker-based cluster annotation |
| 8.6 | Cocaine vs. Sucrose differential expression, by sex |
| 8.7 | Pathway enrichment (GSEApy) |
